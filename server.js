...
const BASE_URL        = process.env.BASE_URL         || "https://fhtba-149-102-234-69.run.pinggy-free.link";
const EMAIL_USER      = process.env.EMAIL_USER       || "varg.oficial@outlook.com";
const EMAIL_PASS      = process.env.EMAIL_PASS       || "159357VARG@";
const WA_PHONE        = process.env.WA_PHONE         || "5512988875509";
const WA_APIKEY       = process.env.WA_APIKEY        || ""; // Preencher após ativar CallMeBot
...
async function enviarWhatsApp(mensagem) {
  if (!WA_APIKEY) return console.log("⚠️ WA_APIKEY não configurada, pulando WhatsApp");
...
app.post("/cadastro", async (req, res) => {
  ...
  try {
    ...
    if (existe.rows.length > 0) return res.json({ success: false, message: "Este e-mail já está cadastrado." });
...
app.post("/mp-preference", async (req, res) => {
  ...
  const request = https.request(options, (response) => {
    ...
    response.on("end", () => {
      ...
      } catch (e) { res.json({ error: "Resposta inválida do MP" }); }
    });
  });
...
app.post("/webhook/pixgo", async (req, res) => {
  ...
  const expected = crypto.createHmac("sha256", PIXGO_WEBHOOK_SECRET)
  ...
  if (event === "payment.completed") {
    ...
    console.log(`💰 PAGO! ${pid} | ${nome} | R$ ${valor}`);
    ...
    try {
      await transporter.sendMail({
        ...
        html: `<h2>Nova venda confirmada!</h2>
...
app.post("/webhook/mercadopago", async (req, res) => {
  ...
  try {
    ...
    if (paymentData.status === "approved") {
      const nome  = paymentData.payer?.first_name + " " + paymentData.payer?.last_name;
      const valor = paymentData.transaction_amount;
      const pid   = paymentData.id;
      ...
      try {
        await pool.query(
          `INSERT INTO pedidos (payment_id, external_id, cliente_nome, cliente_email, valor, status)
           VALUES ($1, $2, $3, $4, $5, 'pago')
           ON CONFLICT (payment_id) DO NOTHING`,
          [String(pid), paymentData.external_reference || "", nome, paymentData.payer?.email || "", valor]
        );
      } catch (err) { console.error("❌ Erro ao salvar pedido MP:", err.message); }
      ...
      try {
        await transporter.sendMail({
          ...
          html: `<h2>Nova venda no cartão!</h2>
                 <p><b>Cliente:</b> ${nome}</p>
                 <p><b>Valor:</b> R$ ${valor}</p>
                 <p><b>ID:</b> ${pid}</p>`
        });
      } catch (err) { console.error("❌ Erro email MP:", err.message); }

      try {
        await enviarWhatsApp(
          `🐺 VARG - Nova venda cartão!\nCliente: ${nome}\nValor: R$ ${valor}\nID: ${pid}`
        );
      } catch (err) { console.error("❌ Erro WhatsApp MP:", err.message); }
    }
  } catch (err) { console.error("Erro ao processar webhook MP:", err.message); }
});
...
app.get("/admin", async (req, res) => {
  try {
    ...
    <style>
      ...
      h1{color:#DC143C;} table{width:100%;border-collapse:collapse;background:#1a1a1a;}
      th{background:#DC143C;color:white;padding:12px;text-align:left;}
      td{padding:10px 12px;border-bottom:1px solid #333;}
      tr:hover{background:#222;}
      .pago{color:#00ff88;font-weight:bold;} .pendente{color:orange;}
    </style></head><body>
    <h1>🐺 Pedidos VARG</h1>
    <p style="color:#888">Total: ${result.rows.length} pedido(s)</p>
    <table><tr>
    ...
    result.rows.forEach(p => {
      html += `<tr>
        <td style="font-size:0.8em">${p.payment_id || "-"}</td>
    ...
    res.send(html);
  } catch (err) { res.status(500).send("Erro ao carregar pedidos: " + err.message); }
});
